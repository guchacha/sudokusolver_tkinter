...
def check_puzzle(outer_list_entry: list):
    """
    Function checks correction of single input, single row, single column and single 3x3 box of inputs. Function defines
    button to pass users inputs to solving function. The button is set in disabled state if users inputs are incorrect.
...
def solve_puzzle(unsolved_list: list):
    """
    Function solves sudoku puzzle and shows solution in the window. In case of no progress (too many unknowns) function
    breaks and shows message box with warning.
